...
const links = [
  ...
  },
  {
    ...
    icon: NoteIcon,
    sublinks: [
      { name: "All Payments", href: "/payment/all-payments" },
...
const secondlinks = [
  { name: "Email Campaign", href: "/email-campaing", icon: ChatIcon },
        ...
        return (
          <div key={link.name}>
            <div
              ...
              className="dashboard-style cursor-pointer"
            >
              <Link href={link.href}>
                <div className="flex items-center w-full justify-between gap-2 pl-5">
            ...
            {isExpanded && !isCollapsed && link.sublinks && (
              <div className="relative ml-9  border-l-2 border-gray-300">
                {link.sublinks.map((sublink) => (
                  <Link key={sublink.name} href={sublink.href} className="block dashboard-style ">
  ...
  return (
    <div className="flex  flex-col border-black lg:h-auto">
      ...
      {secondlinks.map((link) => {
        ...
        const isActive = pathname.startsWith(link.href)
        return (
          <><p className="pl-6 text-sm text-[#727272]">DATABASE</p><Link
            key={link.name}
            href={link.href}
